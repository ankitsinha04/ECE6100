...
uns64   dram_access_sim_rowbuf(DRAM *dram,Addr lineaddr, Flag is_dram_write){
  ...
  int bankId = (lineaddr>>4)%DRAM_BANKS;
  if(dram->perbank_row_buf[bankId].valid== TRUE)
  {
    if(dram->perbank_row_buf[bankId].rowid == lineaddr>>4)
    {
    ...
    else
    {
      ...
      dram->perbank_row_buf[bankId].rowid=lineaddr>>4;
      dram->perbank_row_buf[bankId].valid = TRUE;
    }
  }
  else
  {
    delay = DRAM_T_BUS+ DRAM_T_CAS+ DRAM_T_ACT;
    dram->perbank_row_buf[bankId].valid = TRUE;
    dram->perbank_row_buf[bankId].rowid=lineaddr>>4;
    }
