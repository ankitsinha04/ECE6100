...
uns64 memsys_access_modeDEF(Memsys *sys, Addr v_lineaddr, Access_Type type,uns core_id){
  ...
  uns difac = log2(PAGE_SIZE/CACHE_LINESIZE);
  //printf("%u\n", difac );
  uns offset = v_lineaddr%(PAGE_SIZE/CACHE_LINESIZE);
  uns lineaddr = v_lineaddr/(PAGE_SIZE/CACHE_LINESIZE);

  uns o_lineaddr= memsys_convert_vpn_to_pfn(sys, lineaddr, core_id);

  p_lineaddr = o_lineaddr<<difac | offset;
  ...
  if(type == ACCESS_TYPE_IFETCH){
    Flag outcome = cache_access(sys->icache_coreid[core_id], p_lineaddr, FALSE, core_id);
  ...
  if(type == ACCESS_TYPE_LOAD){
    Flag outcome = cache_access(sys->dcache_coreid[core_id] , p_lineaddr , FALSE , core_id);
    ...
    if(outcome == HIT)
      ...
      if(sys->dcache_coreid[core_id]->last_evicted_line.dirty == 1)
        memsys_L2_access(sys,sys->dcache_coreid[core_id]->last_evicted_line.tag , TRUE , sys->dcache_coreid[core_id]->last_evicted_line.core_id );
    }
  }
  

  if(type == ACCESS_TYPE_STORE){
    Flag outcome = cache_access(sys->dcache_coreid[core_id] , p_lineaddr , TRUE , core_id);
    delay = DCACHE_HIT_LATENCY;
    if(outcome == HIT)
      delay = delay + 0;
    else{
...
uns64   memsys_L2_access(Memsys *sys, Addr lineaddr, Flag is_writeback, uns core_id){
 ...
 uns64 delay = L2CACHE_HIT_LATENCY;
  if(is_writeback == FALSE){
    Flag outcome = cache_access(sys->l2cache,lineaddr,FALSE,core_id);
    if(outcome == HIT)
      	return delay;
      	...
      	if (sys->l2cache->last_evicted_line.dirty==1)
    ...
    Flag outcome = cache_access(sys->l2cache,lineaddr,TRUE, core_id);
    if(outcome == HIT)
     	return delay;
